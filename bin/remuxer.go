...
func (c *DemuxerClient) tryWritingNextBlock() {
	...
	clusterDuration := audioBlock1.timecode - c.outputClusterTimecode
...
func (c *DemuxerClient) writeBlock(block *Block) {
	...
	if rawTimecode > 0x7fff {
		panic(fmt.Sprintf("rawTimecode is too big %d (%d) (%d)\n", rawTimecode, block.timecode, c.outputClusterTimecode))
	}
...
func main() {
	...
	//fmt.Println(minClusterDurationInMS)

	if minClusterDurationInMS > 30000 {
		log.Printf("Invalid minimum cluster duration: 0 > and < 30000\n")
		os.Exit(-1)
	}
